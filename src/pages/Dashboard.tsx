    ...
    current.forEach((c) => {
      (c.products || []).forEach(p => {
        if (isProductExpiringInMonth(p)) {
          expiringSoon.push({ client: c, product: p });
        }
      });
    });
  ...
  return (
    <CRMLayout activePage="dashboard">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="glass-card p-5 animate-fade-in">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-foreground">{t("dashboard.expiring.title")}</h3>
              ...
              stats.expiringSoon.slice(0, 8).map((item, i) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{item.product.policyNumber || "—"} · {item.product.company || "—"}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-warning font-medium">{item.product.expirationDate || "—"}</p>
      ...
      {selectedClient && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedClient(null)}>
          <div className="glass-card max-w-sm w-full p-6 animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-foreground mb-4">Detalle de Póliza</h2>
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-sm py-2 border-b border-border/50">
                ...
                <span className="text-foreground font-medium text-right">{selectedClient.product.policyNumber || "—"}</span>
              </div>
